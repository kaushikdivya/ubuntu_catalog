from flask import Flask, render_template, url_for, redirect, request, flash, session as login_session, make_response
import psycopg2
...
def createUser(login_session):
    newUser = User_info(name = login_session['username'], email = login_session['email'],
        picture = login_session['picture'])
    session.add(newUser)
    session.commit()
    user = session.query(User_info).filter(User_info.email == login_session['email']).one()
    return user.id
...
def check_sub_category_name_id(sub_category, sub_category_id):
    try:
        ...
        category_item = None
    if category_item != None and category_item.name == sub_category:
...
def check_category_sub_category(category, category_id,sub_category, sub_category_id):
    ...
    if catg != None and all_categories != None and catg_item != None:
        category_item = session.query(Category, Category_items).filter(Category.id==Category_items.catagory_id).filter(
            Category_items.id==sub_category_id).filter(Category_items.expiry_date==None).one()
...
def catelog_home():
    if request.method == 'POST':
        ...
        latest_subcatagories = session.query(Category, Category_items
            ).filter(Category.id==Category_items.catagory_id).filter(Category_items.expiry_date==None).order_by(
        ...
        if 'username' not in login_session:
            ...
            return render_template('home_auth.html', all_categories=all_categories,
                        latest_subcatagories=latest_subcatagories)

@app.route('/catalog/<category>/<int:category_id>/items/')
def category_list(category, category_id):
    catg, all_categories = check_category_name_id(category, category_id)
    if catg != None or all_categories != None:
        category_items = session.query(Category_items, Category).filter(
            Category.id==Category_items.catagory_id).filter(Category.id==category_id).filter(Category_items.expiry_date==None)
        if category_items.count() == 0:
            ...
            if catg != [] and catg[0].id == category_id:
                if 'username' not in login_session:
                    ...
                    return render_template('category_items_auth.html', category=category,
                        all_categories=all_categories, category_items=category_items)
            else: 
                return redirect(url_for('catelog_home'))
        else:
...
def sub_category(category, category_id, sub_category, sub_category_id):
    catg, category_item, all_categories = check_category_sub_category(category, category_id, sub_category, sub_category_id)
    if category_item != None and catg != None and all_categories != None:
        if 'username' not in login_session:
            return render_template('item_description.html', category_item=category_item)
        else:
            return render_template('item_description_auth.html', category_item=category_item)
    else:
        return redirect(url_for('catelog_home'))
...
def edit_item(sub_category, sub_category_id):
    ...
    if category_item != None:
        if request.method == 'POST':
            ...
            return redirect(url_for('sub_category', category=category, category_id=category_id, sub_category=sub_category, sub_category_id=sub_category_id))
        else:
            categories = session.query(Category).all()
            category_item = session.query(Category, Category_items).filter(Category.id==Category_items.catagory_id).filter(
                Category_items.id==sub_category_id).one()
            return render_template('edit_item.html', category_item=category_item, categories=categories)
    else:
        return redirect(url_for('catelog_home'))
